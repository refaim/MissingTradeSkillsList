...
function MTSLUI_NPC_LIST_FRAME:Initialise(parent_frame)
...
function MTSLUI_NPC_LIST_FRAME:AddSkillToNpc(npc_id, npc, faction, profession, source_type, rank, skill)
...
----------------------------------------------------------------------------------------------------------
function MTSLUI_NPC_LIST_FRAME:EnhanceNpcData()
            ...
            if sl.trainers then
                for _, npc_id in pairs(sl.trainers.sources) do
                    if npcs[npc_id] == nil then
                        MTSL_TOOLS:Print(MTSLUI_FONTS.COLORS.TEXT.WARNING .. "MTSL: NPC with id " .. npc_id .. " is missing. Please report this bug!")
                    else
...
function MTSLUI_NPC_LIST_FRAME:SetSkillListFrame(skill_list_frame)
    self.skill_list_frame = skill_list_frame
end

function MTSLUI_NPC_LIST_FRAME:UpdateList()
...
function MTSLUI_NPC_LIST_FRAME:FilterListOfNpcs()
    local filtered_npcs = {}
...
function MTSLUI_NPC_LIST_FRAME:IsNpcAvailableForName(npc)
...
function MTSLUI_NPC_LIST_FRAME:IsNpcAvailable(npc)
...
function MTSLUI_NPC_LIST_FRAME:IsNpcAvailableInZone(npc, zone_id)
    local available = true
...
function MTSLUI_NPC_LIST_FRAME:UpdateSlider()
    -- no need for slider
...
function MTSLUI_NPC_LIST_FRAME:UpdateButtons()
    ...
    for i=1,amount_to_show do
        ...
        self.LIST_BUTTONS[i]:SetWidth(self.FRAME_WIDTH - 12 - self.slider_active * 18)
...
function MTSLUI_NPC_LIST_FRAME:HandleSelectedListItem(id)
...
function MTSLUI_NPC_LIST_FRAME:HandleScrollEvent(offset)
...
function MTSLUI_NPC_LIST_FRAME:NoNpcsToShow()
...
function MTSLUI_NPC_LIST_FRAME:Reset()
    -- dselect current npc & button
    self:DeselectCurrentNpcButton()
    self.selected_list_item_index = nil
    self.selected_list_item_id = nil
...
function MTSLUI_NPC_LIST_FRAME:SelectCurrentNpcButton()
...
function MTSLUI_NPC_LIST_FRAME:DeselectCurrentNpcButton()
    if self.selected_button_index ~= nil and
            self.selected_button_index >= 1 and
            self.selected_button_index <= self.MAX_ITEMS_SHOWN_CURRENTLY then
...
function MTSLUI_NPC_LIST_FRAME:HasNpcSelected()
    return self.selected_list_item_id ~= nil
end

function MTSLUI_NPC_LIST_FRAME:ChangeFilters(filters)
    self.filter_values = filters
    self:RefreshList()
end

function MTSLUI_NPC_LIST_FRAME:RefreshList()
    self:DeselectCurrentNpcButton()
    self.selected_list_item_index = nil
    self.selected_button_index = nil
    self.selected_list_item_id = nil
...
function MTSLUI_NPC_LIST_FRAME:ResizeToVerticalMode()
...
function MTSLUI_NPC_LIST_FRAME:ResizeToHorizontalMode()
    -- adjust max items shown
...
function MTSLUI_NPC_LIST_FRAME:RefreshUI()
