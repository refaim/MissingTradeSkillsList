...
MTSL_LOGIC_QUEST = {
    ...
    GetQuestByIds = function(self, ids)
        ...
        while ids[i] ~= nil do
            local quest = MTSL_TOOLS:GetItemFromUnsortedListById(TRADE_SKILLS_DATA["quests"], ids[i])
            -- Check if q started from NPC
            if quest ~= nil then
                if quest.npcs ~= nil then
                    local npcs = MTSL_LOGIC_PLAYER_NPC:GetNpcsByIds(quest.npcs)
                    if npcs == nil  then
                        MTSL_TOOLS:Print(MTSLUI_FONTS.COLORS.TEXT.ERROR .. "MTSL: Could not find NPCs for Quest with id " .. ids[i] .. ". Report this bug!")
                    else
                        -- only 1 NPC possible
                        local npc = npcs[1]
                ...
                MTSL_TOOLS:Print(MTSLUI_FONTS.COLORS.TEXT.ERROR .. "MTSL: Could not find quest with id " .. ids[i] .. ". Report this bug!")
            end
    ...
    GetQuestById = function(self, id)
        return MTSL_TOOLS:GetItemFromUnsortedListById(TRADE_SKILLS_DATA["quests"], id)
    end,
